...
save_train_path = "~/train_word_embed_bioclinical.pt" # path to save bioclinical train bert embeddings
save_dev_path = "~/dev_word_embed_bioclinical.pt" # path to save bioclinical dev bert embeddings
save_test_path = "~/test_word_embed_bioclinical.pt" # path to save bioclinical test bert embeddings
...
bioclinical_model.to(device)

print("Started Embedding Generation: at time:",datetime.now())
train_bert_embed = get_bert_embeddings(input_text = train_text, sent_len = sent_len, tokenizer = bioclinical_tokenizer, max_length_tokens = max_length_tokens, model = bioclinical_model, device = device)
train_embeddings = pad_out_notes(doc_rep = train_bert_embed, sent_len = sent_len, num_sent = num_sent)

dev_bert_embed = get_bert_embeddings(input_text = dev_text, sent_len = sent_len, tokenizer = bioclinical_tokenizer, max_length_tokens = max_length_tokens, model = bioclinical_model, device = device)
dev_embeddings = pad_out_notes(doc_rep = dev_bert_embed, sent_len = sent_len, num_sent = num_sent)

test_bert_embed = get_bert_embeddings(input_text = test_text, sent_len = sent_len, tokenizer = bioclinical_tokenizer, max_length_tokens = max_length_tokens, model = bioclinical_model, device = device)
